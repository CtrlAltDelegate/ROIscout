/**
 * Free data sources for zip_data: Zillow ZHVI/ZORI, HUD FMR, Census ACS.
 * Used by scripts/ingest-zip-data-free-sources.js to populate zip_data table.
 */
...
// Optional: zip prefix to state (first 3 digits) for fallback when state missing
const ZIP_PREFIX_TO_STATE = {
  '006': 'PR', '007': 'PR', '008': 'VI', '009': 'PR',
  '010': 'MA', '011': 'MA', '012': 'MA', '013': 'MA', '014': 'MA', '015': 'MA', '016': 'MA', '017': 'MA', '018': 'MA', '019': 'MA',
  '020': 'MA', '021': 'MA', '022': 'MA', '023': 'MA', '024': 'MA', '025': 'MA', '026': 'MA', '027': 'MA',
  '028': 'RI', '029': 'RI',
  '030': 'NH', '031': 'NH', '032': 'NH', '033': 'NH', '034': 'NH', '035': 'NH', '036': 'NH', '037': 'NH', '038': 'NH', '039': 'NH',
  ...
  '190': 'PA', '191': 'PA', '192': 'PA', '193': 'PA', '194': 'PA', '195': 'PA', '196': 'PA', '197': 'PA', '198': 'PA', '199': 'PA',
  ...
  '260': 'WV', '261': 'WV', '262': 'WV', '263': 'WV', '264': 'WV', '265': 'WV', '266': 'WV', '267': 'WV',
  '268': 'VA',
  ...
  '380': 'TN', '381': 'TN', '382': 'TN', '383': 'TN', '384': 'TN', '385': 'TN', '386': 'TN', '387': 'TN', '388': 'TN', '389': 'TN',
  ...
  '420': 'KY', '421': 'KY', '422': 'KY', '423': 'KY', '424': 'KY', '425': 'KY', '426': 'KY', '427': 'KY', '428': 'KY', '429': 'KY',
  ...
  '540': 'WI', '541': 'WI', '542': 'WI', '543': 'WI', '544': 'WI', '545': 'WI', '546': 'WI', '547': 'WI', '548': 'WI',
  ...
  '670': 'KS', '671': 'KS', '672': 'KS', '673': 'KS', '674': 'KS', '675': 'KS', '676': 'KS', '677': 'KS', '678': 'KS',
  '679': 'KS',
  '680': 'NE', '681': 'NE', '683': 'NE', '684': 'NE', '685': 'NE', '686': 'NE', '687': 'NE', '688': 'NE', '689': 'NE',
  '690': 'NE', '691': 'NE', '692': 'NE', '693': 'NE', '694': 'NE', '696': 'NE',
  '700': 'LA', '701': 'LA', '703': 'LA', '704': 'LA', '705': 'LA', '706': 'LA', '707': 'LA', '708': 'LA', '709': 'LA',
  '710': 'LA', '711': 'LA', '712': 'LA', '713': 'LA', '714': 'LA', '715': 'LA', '716': 'LA', '717': 'LA', '718': 'LA', '719': 'LA',
  ...
  '960': 'CA', '961': 'CA',
  '967': 'HI', '968': 'HI', '969': 'GU', '970': 'OR', '971': 'OR', '972': 'OR', '973': 'OR', '974': 'OR', '975': 'OR', '976': 'OR', '977': 'OR', '978': 'OR', '979': 'OR',
...
/**
 * Parse Zillow-style CSV: header row, then rows with RegionName (or similar) and date columns.
 * Returns array of { zipCode, state (if column), value (latest month numeric) }.
 */
async function parseZillowCsv(filePathOrUrl, options = {}) {
  const { valueColumn, zipColumn = 'RegionName', stateColumn = 'State' } = options;
  let content;
  if (filePathOrUrl.startsWith('http')) {
    const res = await axios.get(filePathOrUrl, { responseType: 'text', timeout: 60000 });
    content = res.data;
  } else {
  ...
  if (valueIdx === -1) {
    ...
    valueIdx = dateCols.length > 0 ? dateCols[dateCols.length - 1].i : header.length - 1;
  }
  const result = [];
...
/**
 * Load Zillow ZHVI (home values) from local CSV or URL.
 * Expected: Zillow "ZIP Code" geography CSV from https://www.zillow.com/research/data/
 */
async function loadZillowZhvi(filePathOrUrl) {
  return parseZillowCsv(filePathOrUrl);
}

/**
 * Load Zillow ZORI (rents) from local CSV or URL.
 */
async function loadZillowZori(filePathOrUrl) {
  return parseZillowCsv(filePathOrUrl);
}

/**
 * Load Census ACS 5-year: median home value (B25077_001E) and median gross rent (B25064_001E) by ZCTA.
 * Requires CENSUS_API_KEY. Returns array of { zipCode, state, medianValue, medianRent }.
 * Census API: geography for ZCTA is "zip code tabulation area" - we request by state to get state in response.
 */
async function loadCensusAcs(options = {}) {
  const apiKey = process.env.CENSUS_API_KEY || options.apiKey;
  const year = options.year || '2022';
  const dataset = `acs/acs5`;
  const baseUrl = `https://api.census.gov/data/${year}/${dataset}`;
  const variables = 'NAME,B25077_001E,B25064_001E';
  const out = [];
  const stateFips = options.stateFips || getStateFipsList();
  for (const state of stateFips) {
    const url = `${baseUrl}?get=${variables}&for=zip%20code%20tabulation%20area:*&in=state:${state}${apiKey ? `&key=${apiKey}` : ''}`;
    try {
      ...
      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        const zip = String(row[zipIdx] || '').replace(/\D/g, '').slice(0, 5);
        if (zip.length !== 5) continue;
        const medianValue = parseCensusValue(row[valueIdx]);
        const medianRent = parseCensusValue(row[rentIdx]);
        const stateAbbr = getStateAbbrFromFips(state);
        if (medianValue > 0 || medianRent > 0) {
          out.push({
            zipCode: zip,
            state: stateAbbr || state,
            medianValue: medianValue > 0 ? medianValue : null,
            medianRent: medianRent > 0 ? medianRent : null,
          });
        }
      }
    } catch (err) {
      if (options.onError) options.onError(state, err);
    }
...
function parseCensusValue(v) {
  if (v == null || v === '' || v === '-' || String(v).toLowerCase() === 'null') return null;
...
function getStateAbbrFromFips(fips) {
  return FIPS_TO_STATE[String(fips).padStart(2, '0')] || null;
}

function getStateFipsList() {
  return Object.keys(FIPS_TO_STATE).filter((f) => !['72'].includes(f));
}

/**
 * Load zip -> state, county from a CSV (optional). Format: zip_code,state,county or zip,state,county.
 */
async function loadZipStateCountyCsv(filePath) {
  const fullPath = path.isAbsolute(filePath) ? filePath : path.resolve(process.cwd(), filePath);
  if (!fs.existsSync(fullPath)) return new Map();
  const content = fs.readFileSync(fullPath, 'utf8');
  const lines = content.split(/\r?\n/).filter(Boolean);
  if (lines.length < 2) return new Map();
  const col = lines[0].split(',').map((c) => c.trim().toLowerCase());
  const zi = col.findIndex((c) => /zip/.test(c));
  const si = col.findIndex((c) => /state/.test(c));
  const ci = col.findIndex((c) => /county/.test(c));
  if (zi === -1 || si === -1) return new Map();
  const map = new Map();
  for (let i = 1; i < lines.length; i++) {
    const row = parseCsvLine(lines[i]);
    const zip = String(row[zi] ?? '').replace(/\D/g, '').slice(0, 5);
    if (zip.length === 5) {
      map.set(zip, {
        state: (row[si] ?? '').toString().trim().slice(0, 2).toUpperCase(),
        county: (ci >= 0 && row[ci] != null) ? (row[ci].toString().trim().slice(0, 100) || null) : null,
      });
    }
  }
  return map;
